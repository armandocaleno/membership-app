        ...
        return $table
            ...
            ->headerActions([
                ...
                ->before(function (CreateAction $action, array $data) {
                    if ($this->getOwnerRecord()->payment_status == 'paid') {
                        Notification::make()
                            ->warning()
                            ->title('Suscripción pagada!')
                            ->body('Esta suscripción no puede recibir pagos porque ya está pagada.')
                            ->persistent()
                            ->send();
                    
                        $action->cancel();
                    }elseif ($this->getOwnerRecord()->saldo() < $data['total']) {
                        Notification::make()
                            ->warning()
                            ->title('Monto no válido!')
                            ->body('El valor a pagar el mayor que el saldo de la suscripción. Cambie el monto e intente de nuevo.')
                            ->persistent()
                            ->send();
